...
```{r echo = FALSE, message = FALSE, warning = FALSE}
# Plot recent presidential polling data over time - reference (9)

poll_and_actual_data |>
  filter(year < 2024 & year > 2000) |>
  ggplot(mapping = aes(x = weeks_left, y =  final_poll, color = party)) +
  geom_line() +
  scale_color_manual(values = c("dodgerblue4", "firebrick1")) +
  scale_x_reverse() +
  geom_hline(mapping = aes(yintercept = pv2p, color = party), linetype = "dotdash") + 
  labs(title = "Polling Data vs Prediction",
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
ggplot(data = poll_and_actual_data, mapping = aes(x = final_poll, y = pv2p, color = party)) +
  ...
  scale_y_continuous(breaks = seq(30, 70, by = 10)) + # Referenced source 3 for this line and previous
...
knitr::kable(top10_polls, digits = 2, col.names = c("Year", "Party", "Candidate", "Two-Party Vote Share", "Final Poll Prediction of Vote Share", "Prediction Error"))
```
