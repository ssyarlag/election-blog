...
coefplot <- read_csv("coefs_final.csv")

knitr::kable(coefplot, col.names = c("Coefficient", "OLS", "Elastic-Net"), align = "c", digits = 4, caption = "Coefficients of OLS and Regularized Models")
```


```{r message = FALSE, warning = FALSE, echo = FALSE}
final_preds <- read_csv("final_pred_map.csv")

final_preds |>
  filter(state %in% c("Arizona", "Georgia", "Michigan", "Nevada", "North Carolina", "Pennsylvania", "Wisconsin")) |>
  select(state, final_pred, final_pred_rep, winner) |>
  knitr::kable(col.names = c("State", "Predicted Vote Share - Harris", "Predicted Vote Share - Trump", "Winner"), align = "c", caption = "Predicted Outcome in Swing States")
```
...
final_preds |>
  group_by(winner) |>
  summarize(sum_elec = sum(electors)) |>
  pivot_wider(names_from = winner, values_from = sum_elec) |>
  knitr::kable(col.names = c("Harris", "Trump"), caption = "Predicted Electoral Outcomes (excluding states without polling data)")
```
...
# Outcome Data
final_pop_vote <- read_csv("state_votes_pres_2024.csv")

# Clean Outcome Data
final_pop_vote <- final_pop_vote |> 
  mutate(FIPS = as.numeric(FIPS), 
         votes_trump = as.numeric(`Donald J. Trump`), 
         votes_harris = as.numeric(`Kamala D. Harris`), 
         votes = as.numeric(`Total Vote`), 
         R_pv = ((votes_trump/votes) * 100) , 
         D_pv = ((votes_harris/votes) * 100), 
         R_pv2p = ((votes_trump/(votes_trump + votes_harris))* 100), 
         D_pv2p = ((votes_harris/(votes_trump + votes_harris))*100)) |> 
  mutate(winner = case_when(votes_trump > votes_harris ~ "REP", 
                            .default = "DEM")) |> 
  select(`Geographic Name`, R_pv, D_pv) |>
  rename(`state` = `Geographic Name`)

# Merge predictions and outcomes data
final_error <- final_preds |>
  left_join(final_pop_vote, by = c("state")) |>
  mutate(final_pred_margin = final_pred - final_pred_rep,
         pred_error_dem = final_pred - D_pv ,
         pred_error_rep = final_pred_rep - R_pv)
```
...
# Mean Absolute Prediction Error (Thank you to Matt Dardet for the code!)
dem_ape <- mean(abs(final_error$D_pv - final_error$final_pred), na.rm = TRUE)
rep_ape <- mean(abs(final_error$R_pv - final_error$final_pred_rep), na.rm = TRUE)

tibble_row("Dem. APE" = dem_ape, "Rep. APE" = rep_ape) |>
  knitr::kable(align = "c")
...
final_error_dem <- final_error |>
  select(state, pred_error_dem) |>
  arrange(abs(pred_error_dem)) |>
  slice(1:10)

final_error_rep <- final_error |>
  select(state, pred_error_rep) |>
  arrange(abs(pred_error_rep)) |>
  slice(1:10)

final_error_dem_swing <- final_error |>
  select(state, pred_error_dem) |>
  arrange(abs(pred_error_dem)) |>
  filter(state %in% c("Arizona", "Michigan", "Pennsylvania", "Wisconsin", "Georgia", "North Carolina", "Nevada"))

final_error_rep_swing <- final_error |>
  select(state, pred_error_rep) |>
  arrange(abs(pred_error_rep)) |>
  filter(state %in% c("Arizona", "Michigan", "Pennsylvania", "Wisconsin", "Georgia", "North Carolina", "Nevada"))

knitr::kable(final_error_dem, col.names = c("State", "Prediction Error (Prediction-Actual)"), align = "c")

# knitr::kable(final_error_margin, col.names = c("State", "Prediction Error"), caption = "Top 10 States with Most Accurate Margin Predictions (Harris - Trump)")
```

Top 10 States with Most Accurate Trump Predictions

```{r message = FALSE, warning = FALSE, echo = FALSE}
knitr::kable(final_error_rep, col.names = c("State", "Prediction Error (Prediction-Actual)"), align = "c")
```

Battleground States by Democratic Prediction Error

```{r message = FALSE, warning = FALSE, echo = FALSE}
knitr::kable(final_error_dem_swing, col.names = c("State", "Prediction Error (Prediction-Actual)"), align = "c")
```
...
knitr::kable(final_error_rep_swing, col.names = c("State", "Prediction Error (Prediction-Actual)"), align = "c")
```
...
error_dem_map <- final_error |>
  select(state, pred_error_dem) |>
  arrange(abs(pred_error_dem)) |>
  left_join(states, by = c("state")) |>
  left_join(states_map, by = "region")

error_rep_map <- final_error |>
  select(state, pred_error_rep) |>
  arrange(abs(pred_error_rep)) |>
  left_join(states, by = c("state")) |>
  left_join(states_map, by = "region")
...
dem_map <- error_dem_map |>
  ggplot(mapping = aes(x=long, y = lat, group = group)) + 
  geom_polygon(aes(fill = pred_error_dem), color = "black") +
  scale_fill_gradient(high = "red2",
                      low = "white") +
  labs(fill = "Democratic Prediction Error", title = "Democratic Prediction Error by State") + 
  theme_void() +
  theme(legend.position = "bottom")
  
rep_map <- error_rep_map |>
  ggplot(mapping = aes(x=long, y = lat, group = group)) + 
  geom_polygon(aes(fill = pred_error_rep), color = "black") +
  scale_fill_gradient(high = "white",
                      low = "red2") +
  labs(fill = "Republican Prediction Error", title = "Republican Prediction Error by State") + 
  theme_void() +
  theme(legend.position = "bottom")
