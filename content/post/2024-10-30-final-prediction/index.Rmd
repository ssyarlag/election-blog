...
```{r echo = FALSE, message = FALSE, warning = FALSE}
# Model Testing: Leave-One-Out for all eight models (2) -- add label for taking colmeans
...
# (15) - Includes new GLM package for elastic net with confidence intervals
...
reg_data <- train_1984 |>
  ungroup() |>
  select(year, state, D_pv, mean_1_wk_poll_support, D_pv_lag1, D_pv_lag2,  InitialCOVI, sc_interact) |>
  drop_na()
...
d.coefplot <- data.frame("OLS" = coef(final_modelOLS)[-1],
...
test$finalrepubpred <- 100-test$finalpred
...
test_final <- test |>
  select(state, finalpred, finalwinner, conf_lowfinal, conf_highfinal)

test_final_output <- test_final |>
  filter(state %in% c("Arizona", "Georgia", "Michigan", "Nevada", "North Carolina", "Wisconsin", "Pennsylvania")) |>
  select(state, finalwinner, finalpred, conf_lowfinal, conf_highfinal)

reg_final_output <- reg_data_test |>
  filter(state %in% c("Arizona", "Georgia", "Michigan", "Nevada", "North Carolina", "Wisconsin", "Pennsylvania")) |>
  mutate(winner = ifelse(final_pred > 50, "DEM", "REP")) |>
  select(state, winner, final_pred)


knitr::kable(test_final_output, col.names = c("State", "Democratic Vote Share", "Predicted Winner", "Low Pred", "High Pred"), align = "c", digits = 2)
```

Regularized Prediction

```{r echo = FALSE, message = FALSE, warning = FALSE}
knitr::kable(reg_final_output, col.names = c("State", "Predicted Winner", "Democratic Vote Share"), align = "c", digits = 2)
```
