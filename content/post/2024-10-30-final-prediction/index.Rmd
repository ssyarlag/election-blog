...
knitr::kable(d.coeftable, col.names = c("Coefficient", "OLS", "Elastic Net"), align = "c")
```
...
# Chunk added after on-time of election model in order to export predictions, training, and test data for use in next blog post - using code from Julia. 2020. “Answer to ‘Export R Data to Csv.’” *Stack Overflow*. <https://stackoverflow.com/a/62017887>.

write.csv(train_1984, "train_1984.csv", row.names = FALSE)
write.csv(d.coeftable, "coefs_final.csv", row.names = FALSE)
write.csv(final_pred_map, "final_pred_map.csv", row.names = FALSE)
```
