...
modelsummary(list("Turnout" = logit_fit_turnout, "Presidential Vote" = logit_fit_presvote), stars = T, output = "kableExtra")
...
d_swing_combined_clean <- mysubset_swing |>
  ...
  mutate(age_subset = case_when(sii_age > 18 & sii_age <= 29 ~ "18-29",
...
gender_plot <- d_swing_combined_clean |> # (6)
  group_by(sii_gender, sii_state) |>
  summarize(mean_turnout = mean(voted_2020, na.rm = T)) |>
  ggplot(mapping = aes(x = sii_gender, y = mean_turnout, color = sii_state)) +
  geom_col(show.legend = FALSE) + 
  geom_label(aes(label = round(mean_turnout,digits = 2))) + 
  theme_minimal() +
  labs(x = "Gender", y = "Turnout Proportion", title = "Turnout by Gender and State") +
...
age_plot <- d_swing_combined_clean |> 
  ...
  geom_label(aes(label = round(mean_turnout, digits = 2))) + 
  theme_minimal() +
  labs(x = "Age Group", y = "Turnout Proportion", title = "Turnout by Age and State") +
  facet_wrap(~sii_state)
...
race_plot <- d_swing_combined_clean |> 
  ...
  geom_label(aes(label = round(mean_turnout,digits = 2))) + 
  theme_minimal() +
  labs(x = "Race", y = "Turnout Proportion", title = "Turnout by Race and State") +
  facet_wrap(~sii_state)
...
educ_plot <- d_swing_combined_clean |> 
  ...
  geom_label(aes(label = round(mean_turnout,digits = 2))) + 
  theme_minimal() +
  labs(x = "Education Level", y = "Turnout Proportion", title = "Turnout by Education Level and State") +
  facet_wrap(~sii_state)
...
urbanicity_plot <- d_swing_combined_clean |> # (7)
  ...
  theme_minimal() +
  labs(x = "Education Level", y = "Turnout Proportion", title = "Turnout by Urbanicity and State") +
  facet_wrap(~sii_state)
